...
var AVLbase = new infoStructure("AVL TREE", "", ["A variation of the Binary Search Tree where nodes also store a balance factor to keep the height of the tree at logN at all times. This allows for the basic operations to have more efiicient worst case than a Binary Search Tree", "Balance Factor: the height of a node's left subtree - the height of a node's right subtree", "The Balance Factor of a node can be -1,0, or 1. If it is outside this range a rotation must be applied to the node."]);
var AVLinsert = new infoStructure("INSERTION", "O(logN)", ["The new value is inserted in the same process as a BST.", "Then the balance factor of each node in the path from the newly inserted node to the root must be checked for any imbalances in the tree. Any unbalaced node is corrected with a rotation."])
var AVLremove = new infoStructure("REMOVAL", "O(logN)", ["The specified value is removed in the same process as a BST.", "Then the balance factor of each node in the path from the removed node to the root must be checked for any imbalances in the tree. Any unbalaced node is corrected with a rotation."])
...
var BHbase = new infoStructure("BINARY HEAP", "", ["A hierarchal structure where nodes must obey a set condition for the type of heap it is. The most importatn node in the heap is always stored at the root.", "Min Heap: The key of a node must be smaller than the key of both of its children, and greater than the key of its parent.", "Max Heap: The key of a node must be greater than the key of both of its children, and smaller than the key of its parent."]);
var BHinsert = new infoStructure("INSERTION", "O(logN)", ["The new value is inserted at the next free position in the level of greatest depth in the heap.", "The node must then be repeatedly compared with its parent and swapped if it does not preserve the conditions of the heap."])
...
var RBinsert = new infoStructure("INSERTION", "O(logN)", ["The new value is inserted in the same process as a BST as a red node.", "The tree must then be checked to see if any of the conditions of the tree are broken and rotations are performed if any unacceptable states are discovered."])
var RBremove = new infoStructure("REMOVAL", "O(logN)", ["The specified value is removed in the same process as a BST, with the replacement node changing to be the colour of the removed node.", "If the replacement node was black, the tree must be checked for any unacceptable states that will be corrected with rotations if discovered."])
...
var FHbase = new infoStructure("FIBONACCI HEAP", "", ["A collection of trees that obey a min order priority meaning that the smllest node is always stored at the root.", "Each node must store its degree, which is the number of children it has at that time."])
var FHinsert = new infoStructure("INSERTION", "O(1)", ["The new value is inserted into the root list. Since the root list is a circular doubly linked list this is a constant time operation."]);
